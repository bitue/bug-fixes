...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <div class=" w-full flex flex-col items-center py-5  bg-gray-100  h-full   text-center rounded-3xl  ">
      <div class="text-center ">
          <img class="product-image h-32 " src=${image}></img>
      </div>
      <h3>${product.title}</h3>
      <p>Category: ${product.category}</p>
      <h2 class="font-bold text-1xl">Price: $ ${product.price}</h2>
      <div>
   
         <p class="space-x-2">
            <span><i class="fas fa-star text-yellow-500"></i></span>  ${rate}  
            <span><i class="fas fa-user-friends text-blue-600"></i> ${count}</span>
         </p> 
      </div>
      <div class="flex justify-around items-center space-x-3">
          <button onclick="addToCart(${product.price})" id="addToCart-btn" class="block rounded-2xl bg-yellow-500 font-bold hover:bg-yellow-600 px-4 py-1 ">add to cart</button>
          <button id="details-btn"class="block rounded-2xl bg-yellow-500 font-bold hover:bg-yellow-600 px-4 py-1 ">Details</button>
